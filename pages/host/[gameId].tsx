...
interface HostState {
  ...
  options?: { id: number; text: string; isCorrect: boolean; answerCount?: number }[];
  remainingMs?: number;
...
const bigBtn: React.CSSProperties = {
  background: '#fff', color: '#1976d2', border: 'none', borderRadius: 10,
  padding: '16px 42px', fontSize: 22, fontWeight: 800, cursor: 'pointer',
...
const HostGame: React.FC = () => {
  ...
  useEffect(() => {
    if (state?.status !== 'title') { titleAdvanced.current = false; return; }
  ...
  useEffect(() => {
    if (state?.status !== 'podium') { setPodiumStage(0); return; }
    if (podiumStage > 0) return;
    const timeouts = [
      setTimeout(() => setPodiumStage(1), 2000),
      setTimeout(() => setPodiumStage(2), 4500),
      setTimeout(() => setPodiumStage(3), 7000),
      setTimeout(() => setPodiumStage(4), 8500),
    ];
    return () => timeouts.forEach(clearTimeout);
  }, [state?.status]); // eslint-disable-line react-hooks/exhaustive-deps

  if (!state) {
    return <Screen><div style={{ color: '#fff', fontSize: 24 }}>Cargando...</div></Screen>;
  }
    ...
    return (
      <Screen>
        ...
        <div
          ...
          style={{ background: '#fff', borderRadius: 12, padding: '18px 32px', marginBottom: 8, textAlign: 'center', cursor: 'pointer', userSelect: 'none' }}
        >
          <div style={{ fontSize: 18, color: '#666' }}>Uníte en <b>{joinUrl}</b> 📋</div>
          <div style={{ fontSize: 56, fontWeight: 900, letterSpacing: 8, color: '#1976d2' }}>{state.code}</div>
          <div style={{ fontSize: 15, fontWeight: 600, color: copied ? '#388e3c' : '#999', minHeight: 20 }}>
            {copied ? '¡URL copiada!' : 'Click para copiar la URL'}
          </div>
        </div>
        <div style={{ color: '#fff', fontSize: 22, margin: '12px 0 20px' }}>
          {state.players?.length || 0} jugador{(state.players?.length || 0) === 1 ? '' : 'es'}
        </div>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, justifyContent: 'center', maxWidth: 900, marginBottom: 32 }}>
          {state.players?.map(p => (
            <div key={p.padron} className="anim-pop-in" style={{
              background: 'rgba(255,255,255,0.92)', borderRadius: 8, padding: '10px 18px', fontWeight: 600, fontSize: 18,
            }}>
              {p.nickname} <span style={{ color: '#888', fontWeight: 400 }}>({p.padron})</span>
            </div>
          ))}
        </div>
        <button onClick={() => advance('lobby')} disabled={(state.players?.length || 0) === 0} style={{
          ...bigBtn,
    ...
    return (
      <Screen>
        <div className="anim-fade-in-scale" style={{ textAlign: 'center' }}>
          <div style={{ color: 'rgba(255,255,255,0.85)', fontSize: 26, marginBottom: 18 }}>¡Prepárense!</div>
          <h1 style={{ color: '#fff', fontSize: 64, margin: 0 }}>{state.quizName}</h1>
          <div style={{ color: 'rgba(255,255,255,0.85)', fontSize: 22, marginTop: 18 }}>{state.totalQuestions} preguntas</div>
        </div>
      </Screen>
    );
  }
  ...
  if (state.status === 'question' || state.status === 'reveal') {
    ...
    return (
      <Screen align="stretch">
        ...
        <div className="anim-slide-up" style={{ background: '#fff', borderRadius: 14, padding: '26px 30px', textAlign: 'center', marginBottom: 22 }}>
          <div style={{ fontSize: 32, fontWeight: 700 }}>{q.text}</div>
          {q.imageUrl && <img src={q.imageUrl} alt="" style={{ maxHeight: 260, maxWidth: '100%', marginTop: 18, borderRadius: 10 }} />}
        </div>

        <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 22 }}>
          {!reveal
            ? <Countdown remainingMs={state.remainingMs || 0} totalMs={q.timeLimit * 1000} />
            : <button onClick={() => advance('reveal')} style={bigBtn}>Siguiente</button>}
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, width: '100%' }}>
          {state.options?.map((o, i) => (
            <div key={o.id} style={{
              ...
              borderRadius: 10, padding: '22px 24px', color: '#fff', fontSize: 24, fontWeight: 700,
              display: 'flex', justifyContent: 'space-between', alignItems: 'center',
              outline: reveal && o.isCorrect ? '5px solid #fff' : 'none',
            }}>
              <span>{reveal && o.isCorrect ? '✓ ' : ''}{o.text}</span>
              {reveal && (
                <span style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <span style={{
                    display: 'inline-block', height: 14, borderRadius: 7, background: 'rgba(255,255,255,0.85)',
      ...
      <Screen>
        ...
        <div style={{ width: '100%', maxWidth: 620 }}>
          {state.leaderboard?.map((p, i) => (
            <div key={p.padron} className="anim-slide-up" style={{
              animationDelay: `${i * 0.12}s`,
              background: '#fff', borderRadius: 10, padding: '14px 22px', marginBottom: 10,
              display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 22, fontWeight: 700,
            }}>
              <span>{p.rank <= 3 ? MEDALS[p.rank - 1] : `${p.rank}.`} {p.nickname} <span style={{ color: '#888', fontWeight: 400, fontSize: 17 }}>({p.padron})</span></span>
              <span style={{ color: '#1976d2' }}>{p.score}</span>
            </div>
          ))}
        </div>
        <button onClick={() => advance('leaderboard')} style={{ ...bigBtn, marginTop: 24 }}>Siguiente</button>
        {error && <ErrorBox text={error} />}
      </Screen>
    );
  }
  ...
  return (
    <Screen>
      ...
      <h1 className="anim-fade-in-scale" style={{ color: '#fff', fontSize: 46, marginBottom: 10 }}>🏆 Podio final</h1>
      <div style={{ color: 'rgba(255,255,255,0.85)', fontSize: 22, marginBottom: 36 }}>{state.quizName}</div>
      {podiumStage === 0 ? (
        <div style={{ textAlign: 'center', height: 340, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
          <div className="anim-drum" style={{ fontSize: 90 }}>🥁</div>
          <div className="anim-pulse" style={{ color: '#fff', fontSize: 30, fontWeight: 700, marginTop: 18 }}>
            Y los ganadores son...
          </div>
        </div>
      ) : (
        <div style={{ display: 'flex', alignItems: 'flex-end', gap: 18 }}>
          {order.map(pos => {
            ...
            return (
              <div key={pos} style={{ width: 220, textAlign: 'center', visibility: visible ? 'visible' : 'hidden' }}>
                {p && visible && (
                  <div className="anim-podium-rise">
                    {isWinner && <div className="anim-pop-in" style={{ fontSize: 40, marginBottom: -6 }}>👑</div>}
                    <div style={{ fontSize: 54 }}>{MEDALS[pos]}</div>
                    <div style={{ color: '#fff', fontSize: isWinner ? 32 : 26, fontWeight: 800, marginBottom: 4 }}>{p.nickname}</div>
                    <div style={{ color: 'rgba(255,255,255,0.85)', fontSize: 17, marginBottom: 10 }}>({p.padron})</div>
                    <div className={isWinner ? 'anim-glow' : undefined} style={{
                      background: pos === 0 ? '#ffd54f' : pos === 1 ? '#e0e0e0' : '#bf8970',
                      height: heights[pos], borderRadius: '10px 10px 0 0',
                      display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: 16,
                      fontSize: 28, fontWeight: 900, color: '#333',
                    }}>
                      {p.score}
                    </div>
                  </div>
      ...
      {podiumStage >= 4 && (
        <div className="anim-slide-up" style={{ marginTop: 36, display: 'flex', gap: 14 }}>
          <button onClick={() => router.push(`/admin/games/${gameId}`)} style={bigBtn}>Ver detalle</button>
          <button onClick={() => router.push('/admin')} style={{ ...bigBtn, background: 'rgba(255,255,255,0.25)', color: '#fff' }}>Volver</button>
        </div>
      )}
    </Screen>
  );
    ...
    <div style={{ position: 'fixed', inset: 0, pointerEvents: 'none', overflow: 'hidden', zIndex: 10 }}>
      {pieces.map((p, i) => (
        <div key={i} style={{
          ...
          width: p.size, height: p.size * (p.round ? 1 : 0.45),
...
const Screen: React.FC<{ children: React.ReactNode; align?: string }> = ({ children, align = 'center' }) => (
  <div style={{
    minHeight: '100vh', background: 'linear-gradient(135deg, #1976d2 0%, #0d47a1 100%)',
...
const ErrorBox: React.FC<{ text: string }> = ({ text }) => (
  <div style={{ marginTop: 18, color: '#fff', background: 'rgba(211,47,47,0.9)', borderRadius: 6, padding: '10px 18px', fontWeight: 500 }}>
    {text}
  </div>
);
