...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (game.status === 'lobby') {
      const { rows: [c] } = await pool.query('SELECT COUNT(*)::int AS n FROM game_players WHERE game_id = $1', [player.game_id]);
      return res.status(200).json({ ...base, playerCount: c.n });
    }

    if (game.status === 'question') {
      const { rows: options } = await pool.query(
        'SELECT id, text FROM question_options WHERE question_id = $1 ORDER BY position',
        [game.question_id]
      );
      const { rows: [answer] } = await pool.query(
        'SELECT 1 FROM game_answers WHERE player_id = $1 AND question_id = $2',
        [player.id, game.question_id]
      );
    ...
    if (game.status === 'reveal') {
      const { rows: correct } = await pool.query(
        'SELECT id FROM question_options WHERE question_id = $1 AND is_correct ORDER BY position',
        [game.question_id]
      );
      const { rows: [answer] } = await pool.query(
        'SELECT is_correct, points, selected_options FROM game_answers WHERE player_id = $1 AND question_id = $2',
        [player.id, game.question_id]
      );
      return res.status(200).json({
        ...base,
        ...
        yourAnswer: answer
          ? { isCorrect: answer.is_correct, points: answer.points, selectedOptionIds: answer.selected_options }
          : null,
      });
    }
