...
import { isValidCharacter, DEFAULT_CHARACTER } from '../../../../lib/characters';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const nickname = String(req.body?.nickname || '')
    .trim()
    .slice(0, 50);
  const avatar = isValidCharacter(req.body?.character) ? req.body.character : DEFAULT_CHARACTER;
  ...
  try {
    const {
      rows: [game],
    } = await pool.query('SELECT id, status FROM games WHERE code = $1', [code]);
    ...
    await pool.query(
      `INSERT INTO game_players (game_id, padron, nickname, avatar, session_token)
       VALUES ($1, $2, $3, $4, $5)
       ON CONFLICT (game_id, padron)
       DO UPDATE SET session_token = EXCLUDED.session_token, nickname = EXCLUDED.nickname, avatar = EXCLUDED.avatar, last_seen_at = now()`,
      [game.id, padron, nickname, avatar, token],
    );
